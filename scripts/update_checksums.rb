...
def update_package_swift(version, checksums, dependency_versions)
  ...
  checksums.each do |name, checksum|
    ...
    package_swift.gsub!(
      ...
      match.gsub(
        %r{url:\s*"https://github\.com/akardas16/google-mlkit-swiftpm/releases/download/[^/]+/},
        "url: \"https://github.com/akardas16/google-mlkit-swiftpm/releases/download/#{version}/"
      ).gsub(
        /checksum:\s*"[^"]+"/,
        "checksum: \"#{checksum}\""
      )
